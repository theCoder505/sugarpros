...
function showMessage(passedThis) {
    ...
    let token = $(".token").val();
    $.ajax({
        url: '/provider/fetch-related-chats',
        type: 'POST',
        data: {
            _token: token,
            message_with: dataID,
                    ...
                    if (isSent) {
                        ...
                        messageData += `<div class="flex flex-col items-end gap-1 mb-2">
                            ...
                            <span class="text-xs text-[#000000] time_details hidden">
                                ${formattedTime} <i class="fas fa-check-double text-[#000000] ml-1"></i>
                            </span>
                        </div>`;
                    }
                });
...
function sendMessage(passedThis) {
    ...
    // do ajax work 
    let token = $(".token").val();
    let send_text_to = $(".send_text_to").val();
    $.ajax({
        ...
        success: function (response) {
            if (response.message == 'success') {
                ...
                if (chatItem.length) {
                    ...
                    chatItem.children(".provider_details").children(".name_section").children(".patient_message").html(previewText);
                    chatItem.children(".timeandseen").children(".msg_time").html(formattedTime);
    ...
    reader.onload = function (e) {
        const imageMessage = `
            <div class="flex flex-col items-end gap-1 mb-2">
                <div class="bg-[#2889AA] p-2 rounded-lg chat_message">
        ...
        $.ajax({
            url: '/provider/send-image-message',
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
            success: function (response) {
                if (response.message == 'success') {
                    ...
                    if (chatItem.length) {
                        ...
                        chatItem.children(".provider_details").children(".name_section").children(".patient_message").html('You sent a picture');
                        chatItem.children(".timeandseen").children(".msg_time").html(formattedTime);
                        const usersList = chatItem.closest('.users_list');
                        if (usersList.length) {
                            chatItem.prependTo(usersList);
