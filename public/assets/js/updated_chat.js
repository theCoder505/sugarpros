...
function handleOnlineStatus(data) {
    const currentChatId = $(".send_text_to").val();
    if (currentChatId) {
        const isOnline = data.onlineUsers.includes(currentChatId);
        $(".activity_status").removeClass('text-gray-400 text-green-500')
            .addClass(isOnline ? 'text-green-500' : 'text-gray-400')
            .text(isOnline ? 'Online' : 'Offline');

        // Update in chat list if available
        const chatItem = $(`.chat-item[data-id="${currentChatId}"]`);
        if (chatItem.length) {
            const statusIndicator = chatItem.find('.image_section img:last-child');
            if (isOnline) {
