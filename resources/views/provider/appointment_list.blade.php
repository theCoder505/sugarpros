                            ...
                            <tr class="border-b border-[#000000]/10 appointment-row" 
                                ...
                                <td class="px-4 py-4 max-w-[100px]">
                                    ...
                                    @elseif($statusClass == 'upcoming')
                                        <span class="status-badge bg-purple-100 text-purple-800">{{ $statusText }}</span>
                                    @elseif($statusClass == 'missed')
                                        <span class="status-badge bg-red-100 text-red-800">{{ $statusText }}</span>
                                    @elseif($statusClass == 'unset')
                                        <span class="status-badge bg-gray-100 text-gray-800">{{ $statusText }}</span>
    ...
    <script>
        ...
        function filterAppointments(selectedType) {
            ...
            if (selectedType === 'all') {
                ...
                table.rows().every(function() {
                    var row = this.node();
                    if ($(row).data('status') === selectedType) {
                        $(row).show();
                    }
                });
            }
        ...
        $(document).ready(function() {
            ...
            table.on('search.dt', function() {
                // If there's a search term, show all rows that match search AND current filter
                var searchTerm = table.search();
                var currentFilter = $('#appointmentTypeFilter').val();
                
                if (searchTerm) {
                    // Let DataTables handle the search, then apply our filter
                    setTimeout(function() {
                        if (currentFilter !== 'all') {
                            table.rows().every(function() {
                                var row = this.node();
                                if ($(row).data('status') !== currentFilter && $(row).is(':visible')) {
                                    $(row).hide();
                                }
                            });
                        }
