...
<body>
    ...
    <script>
        let socket;
        const userId = "{{ Auth::user()->patient_id }}";
        ...
        function showNewMessageAppeared() {
            if ($(".message_topbar").length === 0 || $(".message_topbar").hasClass("hidden")) {
                ...
                $span.html(new_tot);
                toastr.info('<a href="/chats" style="color:#fff;text-decoration:underline;">Message From Provider</a>');
            }


            if ($(".chating_section").length) {
                let unread_total = Number($(".unread_tol").html());
                let new_unread_total = (unread_total + 1);
                $(".unread_tol").html(new_unread_total);
